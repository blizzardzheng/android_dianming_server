...
//							"'"+resObject.getString("resourceStatus")+"',"
	 ...
	 * @return
	 */
	public static boolean insertOrders(JSONObject resObject  ) {
		...
		try {
			stmt = conn.createStatement();
			String sql = "insert into orders_tb (dianhua,neirong,user_name, resource_name, rid , uid) " +
					"values ('"+resObject.getString("dianhua")+"','"+resObject.getString("neirong")+"','" +
					resObject.getString("user_name")+"' , '" + resObject.getString("resource_name")+"', " +
					resObject.getInt("uid") + " ," +   resObject.getInt("uid")+")" ;
...
//	}
	 ...
	 * @return
	 */
	public static JSONArray getUserByUserId2(String userId) {
	 ...
	 * @return
	 */
	public static JSONArray getOrdersByUserName( String userName ) {
		JSONArray resourceArrays = new JSONArray();
		ResultSet rs = null;
		Statement stmt = null;
		Connection conn = manager.getConnection();
		String sql = "select *  from   orders_tb  where user_name = '" + userName + "'  order by oid desc ";
		try {
			stmt = conn.createStatement();
			rs = stmt.executeQuery(sql);
			while(rs.next()){
				JSONObject object = new JSONObject();
				object.put("oid", rs.getInt("oid"));
				object.put("dianhua", rs.getString("dianhua"));
				object.put("neirong", rs.getString("neirong")); 
				object.put("resource_name", rs.getString("resource_name")); 
				object.put("rid", rs.getInt("rid")); 
				object.put("uid", rs.getInt("uid"));  
				resourceArrays.put(object);
			}
	 ...
	 * @return
	 */
	public static JSONArray getAllCommentsByRid(String rid) {
		JSONArray resourceArrays = new JSONArray();
		ResultSet rs = null;
		Statement stmt = null;
		Connection conn = manager.getConnection();
		String sql = "select comment_tb.*,user_tb.user_username  from comment_tb , user_tb  where comment_tb.rid =" + rid + "  and comment_tb.uid=user_tb.uid  ";
		try {
			stmt = conn.createStatement();
			rs = stmt.executeQuery(sql);
			while(rs.next()){
				...
				object.put("comment_date", rs.getString("comment_date"));
				object.put("userName", rs.getString("user_username"));  
